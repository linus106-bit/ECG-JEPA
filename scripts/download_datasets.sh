...
echo "모든 다운로드 완료!"
echo ""
echo "다음 단계: dump_data 실행"
echo ""
echo "  python -m scripts.dump_data --data-dir \"$PTB_XL_DIR\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$MIMIC_DIR\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$CINC_DIR/chapman_shaoxing\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$CINC_DIR/cpsc_2018\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$CINC_DIR/cpsc_2018_extra\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$CINC_DIR/georgia\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$CINC_DIR/ningbo\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$CINC_DIR/ptb\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$CINC_DIR/st_petersburg_incart\" --verbose"
echo "  python -m scripts.dump_data --data-dir \"$CODE15_DIR\" --verbose"
echo ""
echo "  # St-Petersburg, CODE-15는 추가 전처리 필요:"
echo "  python -m scripts.preprocess_data_dump --data \"$CINC_DIR/st_petersburg_incart.npy\" --interpolate-nans --remove-baseline-wander"
echo "  python -m scripts.preprocess_data_dump --data \"$CODE15_DIR.npy\" --interpolate-nans --remove-baseline-wander"
echo ""
echo "  # PTB-XL pre-training용 분할:"
echo "  python -m scripts.split_ptb_xl --data-dir \"$PTB_XL_DIR\" --folds 1 2 3 4 5 6 7 8"
